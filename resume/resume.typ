...
#text(red, "RADIUS — Architecture (Supplicant => NAS => Serveur)"): *Supplicant*: user/équipement demandant accès (PC, smartphone, routeur). *NAS (Network Access Server)*: équipement réseau en bordure recevant demande de connexion (stocke jamais identifiants, délègue entièrement décision d'accès au serveur RADIUS). *Serveur RADIUS*: vérifie identifiants dans LDAP/AD (bases de données centralisées des utilisateurs d'entreprise), retourne décision aveces attributs de session (VLAN attribué, adresse IP, politique QoS, durée max). RFC 2865/2866. UDP port 1812 (authentification) et 1813 (accounting).
